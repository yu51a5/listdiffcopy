...
class ScrappingMediumPage(SomeAction1):
  ...
  def __init__(self, url, StorageType, path, kwargs_storage, backup_name_override=None, main_tag='article'):
    ...
    with StorageWeb() as self.instanceStorageWeb:
      response_text = self.instanceStorageWeb.get_contents(filename=url, use_content_not_text=False)
      ...
      back_up_content = (SomeAction1.put_together_framed_message(message='Backing up ' + url)
                         + article_text
      ...
      StorageWeb.create_fake_directory(dir_name=assets_dir,
                                       urls_content=all_pic_urls, 
                                       fake_filename_contents_text={os.path.basename(assets_dir)+'.txt' : back_up_content})
